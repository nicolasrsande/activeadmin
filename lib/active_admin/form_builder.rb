...
module ActiveAdmin
  ...
  class HasManyBuilder < SimpleDelegator
    ...
    def has_many_actions(form_builder, contents)
      if form_builder.object.new_record?
        contents << template.content_tag(:li) do
          template.link_to '<i class="fas fa-trash"></i> '.html_safe + I18n.t('active_admin.has_many_remove'), "#", class: 'button has_many_remove'
        end
    ...
    def js_for_has_many(class_string, &form_block)
      ...
      text = new_record.is_a?(String) ? new_record : '<i class="fas fa-plus"></i> '.html_safe + I18n.t('active_admin.has_many_new', model: assoc_name.human)
