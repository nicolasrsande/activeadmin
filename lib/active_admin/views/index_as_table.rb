      ...
      class IndexTableFor < ::ActiveAdmin::Views::TableFor
        ...
        def defaults(resource, options = {})
          ...
          if controller.action_methods.include?('show') && authorized?(ActiveAdmin::Auth::READ, resource)
            item '<i class="fas fa-eye fa-2x"></i> '.html_safe, resource_path(resource), class: "view_link #{options[:css_class]}", title: localizer.t(:view)
          end
          if controller.action_methods.include?('edit') && authorized?(ActiveAdmin::Auth::UPDATE, resource)
            item '<i class="fas fa-edit fa-2x"></i> '.html_safe, edit_resource_path(resource), class: "edit_link #{options[:css_class]}", title: localizer.t(:edit)
          end
          if controller.action_methods.include?('destroy') && authorized?(ActiveAdmin::Auth::DESTROY, resource)
            item '<i class="fas fa-trash fa-2x"></i> '.html_safe, resource_path(resource), class: "delete_link #{options[:css_class]}", title: localizer.t(:delete),
